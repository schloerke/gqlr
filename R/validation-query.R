      ...
      if (definition$operation %in% c("query", "mutation")) {
        ...
        if (is.null(name_val)) {
          ...
          if (isTRUE(seen_names[[name_val]])) {
            oh$error_list$add(
              "5.1.1.1",
              "document definition has duplicate request name: ",
              name_val,
...
validate_field_selections <- function(document_obj, ..., oh) {
  for (operation in document_obj$definitions) {
    if (!is.null(operation$operation)) {
      ...
      var_validator <- VariableValdationHelper$new(
  ...
  for (selection_obj in selection_obj_list) {
    if (inherits(selection_obj, "FragmentSpread")) {
      ...
      if (!(selection_obj$name$value %in% obj_field_names)) {
        if (selection_obj$name$value == "__typename") {
          ...
          top_level &&
            (selection_obj$name$value == "__schema" ||
              selection_obj$name$value == "__type")
        ) {
          ...
          oh$error_list$add(
            ...
            "Not allowed to ask for fields: ",
            str_c(get_name_values(list(selection_obj)), collapse = ", "),
    ...
    matching_obj_field <-
      if (field_name == "__typename") {
        ...
        object_field_list[[which(obj_field_names == field_name)]]
      }
    ...
    if (!is.null(selection_obj$selectionSet)) {
      ...
      matching_obj <- get_object_interface_or_union(
        matching_type$name,
        oh$schema
      )
      if (is.null(matching_obj)) {
        ...
        oh$error_list$add(
          "5.2.3",
          "unknown object definition for field: '",
          selection_obj$name$value,
          "'.",
      ...
      if (inherits(selection_obj, "Field")) {
        ...
        if (!is.null(matching_obj)) {
          oh$error_list$add(
            ...
            " Missing children fields for field: '",
            selection_obj$name$value,
            "'.",
            loc = selection_obj$loc
          )
...
validate_directives <- function(
  ...
  if (length(directives) > 0) {
    directive_names <- lapply(directives, `[[`, "name") %>%
      lapply(`[[`, "value") %>%
      unlist()
    # 5.6.3
...
validate_directive <- function(
  ...
  directive_definition$locations %>%
    lapply(`[[`, "value") %>%
    unlist() -> directive_possible_locations
  ...
  if (!(parent_cur_location %in% directive_possible_locations)) {
    oh$error_list$add(
      ...
      "' situation.",
      " Can only be used in: '",
      str_c(directive_possible_locations, collapse = "', '"),
      "'",
      loc = parent_obj$loc
    )
  }
