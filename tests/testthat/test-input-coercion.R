...
test_that("input-coercion - Int", {
  ...
  "{ arguments { intArgField(intArg: \"5\") } }" %>%
    expect_err("Expected type ")
...
test_that("input-coercion - Float", {
  ...
  "{ arguments { floatArgField(floatArg: true) } }" %>%
    expect_err("Expected type ")
  "{ arguments { floatArgField(floatArg: \"5.0\") } }" %>%
    expect_err("Expected type ")
})
...
test_that("input-coercion - String", {
  ...
  "{ arguments { stringArgField(stringArg: 5) } }" %>%
    expect_err("Expected type ")
  "{ arguments { stringArgField(stringArg: 5.0) } }" %>%
    expect_err("Expected type ")
  "{ arguments { stringArgField(stringArg: true) } }" %>%
    expect_err("Expected type ")
})
...
test_that("input-coercion - Boolean", {
  ...
  "{ arguments { booleanArgField(booleanArg: \"5\") } }" %>%
    expect_err("Expected type ")
  "{ arguments { booleanArgField(booleanArg: 5) } }" %>%
    expect_err("Expected type ")
  "{ arguments { booleanArgField(booleanArg: 5.0) } }" %>%
    expect_err("Expected type ")
})
